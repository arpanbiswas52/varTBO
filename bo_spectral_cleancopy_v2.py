...
def generate_targetobj(X, spec_norm, lowres_image, V, m):
    ...
    if (ind < X.shape[0]):
        ...
        if Rate == "Bad":
            st.session_state.vote[st.session_state.ind-1,0] = 0
            st.session_state.wcount_good = st.session_state.wcount_good + st.session_state.vote[st.session_state.ind-1,0]
            st.write('Vote given for current spectral', Rate)
            #st.write(st.session_state.key)

        elif Rate == "Good":
            st.session_state.vote[st.session_state.ind-1,0] = 1
            ...
            st.session_state.wcount_good = st.session_state.wcount_good + st.session_state.vote[st.session_state.ind-1,0]
            st.session_state.target_func = (((1-newspec_wt)*st.session_state.target_func*(st.session_state.wcount_good-st.session_state.vote[st.session_state.ind-1,0]))\
                           + (newspec_wt*st.session_state.vote[st.session_state.ind-1,0]*spec_norm[idx1, idx2, :]))/(((st.session_state.wcount_good-st.session_state.vote[st.session_state.ind-1,0])*(1-newspec_wt))\
                           + (st.session_state.vote[st.session_state.ind-1,0]*newspec_wt))
            #st.write(st.session_state.key)

        else:
            st.session_state.vote[st.session_state.ind-1,0] = 2
            #wcount_good = wcount_good + vote
            st.write('Vote given for current spectral', Rate)
            newspec_wt = 1
            if ((st.session_state.wcount_good) > 0): #Only if we already have selected good spectral in early iterations
